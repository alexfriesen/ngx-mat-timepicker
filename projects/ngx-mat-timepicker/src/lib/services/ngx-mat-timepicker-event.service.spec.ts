...
describe('NgxMatTimepickerService', () => {
  ...
  it('should dispatch click event', () => {
    eventService.backdropClick.subscribe((event) =>
  ...
  it('should dispatch keydown event', () => {
    eventService.keydownEvent.subscribe((event) =>
  ...
  it('should throw error if wrong event type', () => {
    expect(() =>
      eventService.dispatchEvent({ type: 'mouseout' } as MouseEvent),
    ).toThrowError('no such event type');
  });
});
