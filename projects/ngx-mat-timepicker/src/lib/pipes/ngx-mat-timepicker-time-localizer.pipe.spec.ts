...
describe('NgxMatTimepickerTimeLocalizerPipe', () => {
  ...
  it('should return hour in numeric format', () => {
    ...
    hours.forEach((hour) => {
      const expected = DateTime.fromObject({ hour: hour })
  ...
  it('should return hour in 2-digit format when isKeyboardEnabled is true', () => {
    ...
    expect(pipe.transform(hour, NgxMatTimepickerUnits.HOUR, true)).toBe(
      expected,
    );
  });
  ...
  it('should return minute in 2-digit format', () => {
    ...
    minutes.forEach((minute) => {
      const expected = DateTime.fromObject({ minute: minute })
        .setLocale(defaultLocale)
        .toFormat('mm');
      expect(
        pipe.transform(
  ...
  it('should throw an error when unexpected NgxMatTimepickerUnits is provided', () => {
    try {
      pipe.transform(1, undefined);
    } catch (e: unknown) {
      expect(e instanceof Error).toBeTruthy();
      expect((e as Error).message).toBe(
        'There is no Time Unit with type undefined',
      );
    }
  });
  ...
  it('should throw an error when cannot format provided time', () => {
    ...
    try {
      ...
      expect(e instanceof Error).toBeTruthy();
      expect((e as Error).message).toBe(
        `Cannot format provided time - ${time} to locale - ${defaultLocale}`,
      );
    }
  });
});
